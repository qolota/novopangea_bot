...
const URLS = {
    ...
    [API_ENDPOINTS.TEMPLATES_STATS]: 'https://wax.api.atomicassets.io/atomicassets/v1/templates/${template_id}/stats',
...
const fetchAtomichub = async ({
    ...
    switch (type) {
        case API_ENDPOINTS.LOGS:
            ...
            break;
        case API_ENDPOINTS.TEMPLATES_STATS:
            url = url.replace('${template_id}', params.template_id);
            _param = _.omit(params, 'template_id');
            break;
    }
    if (url == null) {
    ...
    console.log(`Loading [${type}] page=[${_param.page || 'N/A'}] ${atomicAssetsUrl}`);
